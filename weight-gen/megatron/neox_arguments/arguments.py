...
class NeoXArgs(*BASE_CLASSES):
    ...
    def __post_init__(self):
        ...
        hostname = f"{gethostname()}_{time.time()}"
        ...
        self.save = os.path.join(self.base_dir, self.expr_name, hostname, self.save)
    ...
    def consume_deepy_args(cls, input_args=None):
        ...
        # neox_args.expr_name = f"GPT_experts-{neox_args.moe_num_experts}-topk-{neox_args.moe_top_k}-layers{neox_args.num_layers}-heads-{neox_args.num_attention_heads}_{time.time()}"
        
        # neox_args.tensorboard_dir = os.path.join(neox_args.base_dir, neox_args.expr_name, hostname, neox_args.tensorboard_dir)
        # neox_args.log_dir = os.path.join(neox_args.base_dir, neox_args.expr_name, hostname, neox_args.log_dir)
        # neox_args.save = os.path.join(neox_args.base_dir, neox_args.expr_name, hostname, neox_args.save)
    ...
    def enable_logging(self):
        ...
        if self.log_dir:
            ...
            stdout_file = f"{file_prefix}_{hostname}_stdout.txt"
            stderr_file = f"{file_prefix}_{hostname}_stderr.txt"
            #if not os.path.isfile(stdout_file):
            Tee(stdout_file, err=False)
            #if not os.path.isfile(stderr_file):
            Tee(stderr_file, err=True)
