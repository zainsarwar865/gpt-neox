...
class GPT2ModelPipe(PipelineModule, torch.nn.Module):
    ...
    def __init__(
        ...
        self.specs = []
        if neox_args.moe_router_type in ["topk", "sinkhorn"]:
            self.buffer = None
        else:
            self.buffer = torch.zeros((neox_args.seq_length * neox_args.train_micro_batch_size_per_gpu * neox_args.moe_num_experts, neox_args.hidden_size), requires_grad=False).to('cuda', dtype=torch.bfloat16)
        # self.scores = torch.zeros(neox_args.moe_num_experts, neox_args.seq_length * neox_args.train_micro_batch_size_per_gpu // 64, neox_args.seq_length * neox_args.train_micro_batch_size_per_gpu // 64).to('cuda', dtype=torch.bfloat16)
        # self.approx = torch.zeros(neox_args.moe_num_experts, neox_args.seq_length * neox_args.train_micro_batch_size_per_gpu // 64, neox_args.hidden_size).to('cuda', dtype=torch.bfloat16)
