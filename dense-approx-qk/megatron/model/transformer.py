...
from megatron.model.moe_dense import ParallelDroplessMoE
...
class ParallelSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None):
        ...
        return output, bias, queries, keys
...
class ParallelTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        # Dense MLP selector
        def get_mlp(mlp_type, neox_args, **kw):
            if mlp_type == "regular":
                return ParallelMLP(
                    neox_args=neox_args,
                    init_method=init_method,
        ...
        if self.num_experts <= 1:
            self.mlp = get_mlp(neox_args.mlp_type, neox_args)
        # Dropless MoE MLP
        else:
    ...
    def forward(self, x, attention_mask, layer_past=None):
        ...
        if self.gpt_j_residual:
            ...
            residual = x
            # x = x + attn(ln1(x))
            attention_output, attention_bias, queries, keys = self.attention(
            ...
            mlp_output, mlp_bias = self.mlp(layernorm_output, queries=queries, keys=keys)
