...
class SinkhornRouter(torch.nn.Module):
    ...
    def __init__(
        ...
        self.global_lora_routing_counts = torch.zeros(neox_args.moe_num_experts)
...
class TopKTokenChoiceRouterLoRa(torch.nn.Module):
    ...
    def __init__(
        ...
        #self.global_lora_routing_counts = torch.zeros(self.total_loras)
    ...
    def forward(self, x, grouped_gemm_batch_sizes):
        ...
        if self.expert_parallel_rank == 0:
            ...
            # global_lora_routing_counts = torch.distributed.all_reduce(
            #     lora_routing_counts,
            #     group=self.data_parallel_group,
            #     op=torch.distributed.ReduceOp.SUM,
            #     async_op=True,
