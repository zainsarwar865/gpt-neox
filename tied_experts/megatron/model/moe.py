...
class ParallelDroplessMLP(torch.nn.Module):
    ...
    def __init__(
        ...
        if neox_args.mlp_type == "regular":
            self.mlp = experts
            #print(f"ID of experts at rank {get_model_parallel_rank()} at layer : {layer_idx} for process ID : {os.getpid()}is  : {id(self.mlp)}" )
    ...
    def permute_and_compute(
        ...
        #print(f"Layer {self.layer_idx} : {local_tokens_per_expert}")
