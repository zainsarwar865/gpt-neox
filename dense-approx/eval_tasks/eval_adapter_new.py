...
#model dict dict_keys(['training', '_parameters', '_buffers', '_non_persistent_buffers_set', '_backward_pre_hooks', '_backward_hooks', '_is_full_backward_hook', '_forward_hooks', '_forward_hooks_with_kwargs', '_forward_hooks_always_called', '_forward_pre_hooks', '_forward_pre_hooks_with_kwargs', '_state_dict_hooks', '_state_dict_pre_hooks', '_load_state_dict_pre_hooks', '_load_state_dict_post_hooks', '_modules', 'dont_change_device', 'client_optimizer', 'client_lr_scheduler', 'training_data', 'collate_fn', 'mpu', 'all_to_all_group', 'data_parallel_group', 'global_steps', 'global_samples', 'micro_steps', 'skipped_steps', 'gradient_average', 'warn_unscaled_loss', 'config', '_config', 'loaded_checkpoint_mp_world_size', 'loaded_checkpoint_dp_world_size', 'enable_backward_allreduce', 'progressive_layer_drop', 'eigenvalue', 'block_eigenvalue', 'gas_boundary_ctr', 'dist_backend', 'has_moe_layers', 'num_experts', 'gate_modules', 'moe_layers', '_step_applied', '_global_grad_norm', 'use_ds_comm', 'checkpoint_engine', '_is_gradient_accumulation_boundary', 'scale_wrt_gas', 'losses', 'param_names', 'local_rank', 'device', 'world_size', 'global_rank', 'monitor', 'pipeline_parallelism', 'module', 'local_all_to_all_group', 'dp_world_size', 'seq_data_parallel_group', 'seq_dp_world_size', 'mp_world_size', 'expert_parallel_group', 'expert_data_parallel_group', 'sequence_parallel_size', 'timers', 'tput_timer', 'training_dataloader', 'optimizer', 'basic_optimizer', 'lr_scheduler', 'sparse_tensor_module_names', 'save_non_zero_checkpoint', 'save_zero_checkpoint', 'engine_timers', 'flatten', 'unflatten', 'total_params']
...
class EvalHarnessAdapter(HFLM):
    ...
    def __init__(self, model, forward_step_fn, neox_args, batch_size=None):
        ...
        # print_rank_0('model dict', {k:type(v) for k,v in model.__dict__.items()})
    ...
    def run_eval(
        ...
        utils.eval_logger.setLevel(logging.DEBUG)
        was_training = self.model.training
        self.model.eval()
        in_micro_batches = (
            self.neox_args.train_micro_batch_size_per_gpu #self.model.micro_batches
        ...
        print_rank_0("eval tasks before pattern match",eval_tasks)
        ...
        utils.eval_logger.debug("[warning] COMMENTED CODE NO DOWNLOADIN IN CAPITAL ONE")
        # COMMENTED CODE NO DOWNLOADIN IN CAPITAL ONE
        ...
        print("eval tasks before tasks.get_task_dict",eval_tasks)
        task_dict = tasks.get_task_dict(eval_tasks)
