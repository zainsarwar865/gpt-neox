...
class ParallelGroupedMLP(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, tokens_per_expert: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            # tokens_per_lora_log[self.layer_num].append(tokens_per_lora.cpu())
            # # print(f"tokens_per_lora : {tokens_per_lora}, self.args.mlp_layer_number, {self.layer_num}")

            # if self.layer_num == 0:
            #     forward_step_counter += 1

            #     # Every 100 steps, save all logs to disk
            #     if forward_step_counter % 100 == 0:
            #         os.makedirs("tokens_per_lora_dumps", exist_ok=True)
            #         filename = f"tokens_per_lora_dumps/step_{forward_step_counter}.pt"
            #         torch.save(dict(tokens_per_lora_log), filename)
            #         print(f"[LoRA] Saved tokens_per_lora at step {forward_step_counter} to {filename}")

            #         # Clear log after saving
            #         tokens_per_lora_log.clear()
