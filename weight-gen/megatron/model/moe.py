...
from .moe_mlp import ParallelGroupedLLaMAMLP, ParallelGroupedMLP, ParallelWeightNetMLP
...
class ParallelDroplessMLP(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.expert_embedding_mode:
            ...
            self.WeightNet = ParallelWeightNetMLP(
                neox_args=neox_args,
                init_method=init_method,
                output_layer_init_method=output_layer_init_method,
            )
    ...
    def permute_and_compute(
        ...
        w1_emb, w2_emb = self.experts_embedding(self.mlp)
        output_parallel = self.mlp(input_parallel, local_tokens_per_expert, self.WeightNet, w1_emb, w2_emb, bin_ids)
