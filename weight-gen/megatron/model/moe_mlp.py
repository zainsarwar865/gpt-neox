...
class ParallelWeightNetMLP(torch.nn.Module):
    def __init__(
        self,
        neox_args: NeoXArgs,
        init_method,
...
class ParallelGroupedMLP(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, tokens_per_expert: torch.Tensor, WeightNet: ParallelWeightNetMLP, w1_emb : torch.tensor, w2_emb : torch.tensor, bin_ids : torch.tensor):
        ...
        w1_lowrank_row = w1_lowrank[:, :self.hidden_size]
        w1_lowrank_col = w1_lowrank[:, self.hidden_size:]
        w2_lowrank_row = w2_lowrank[:, self.hidden_size:]
        w2_lowrank_col = w2_lowrank[:, :self.hidden_size]
