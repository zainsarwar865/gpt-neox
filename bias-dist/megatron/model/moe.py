...
from .router_aux import TopKTokenChoiceRouter, SinkhornRouter, SparseMixerRouter
from .expert_weight_embedding import ExpertEmbedding
...
class ParallelDroplessMLP(torch.nn.Module):
    ...
    def __init__(
        ...
        self.experts_embedding = ExpertEmbedding(neox_args, init_method, output_layer_init_method)
    ...
    def permute_and_compute(
        ...
        #print("Shape of local_tokens_per_expert", local_tokens_per_expert.shape)
        # if torch.cuda.current_device() == 0:
        #     print(f"{torch.cuda.current_device()}: local_tokens_per_expert {local_tokens_per_expert}, global tokens {tokens_per_expert}")

        # BiasNet stuff goes here
        #bin_ids are topk-ids for each token (tokens are replicated to account for top-k > 1)
        ########        
        # Send input_parallel, expert_indices and c

        #expert_token_embeddings = self.experts_embedding(input_parallel, bin_ids, self.mlp)
        ########
        
        #expert_embeddings = self.experts_embedding(self.mlp)
        #expert_embeddings_idx = self.experts_embeddings[bin_ids]
        # up_emb, down_emb = self.experts_embedding(self.mlp)
        # expert_token_embeddings = torch.cat((expert_embeddings_idx, input_parallel), dim=-1)
        # print("expert_token_embeddings", expert_token_embeddings.shape)
        # bias_terms = self.BiasNet(expert_token_embeddings)

        # Perform the expert computation for this rank's experts
        output_parallel = self.mlp(input_parallel, local_tokens_per_expert, bin_ids)
