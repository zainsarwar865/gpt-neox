...
class SinkhornRouter(torch.nn.Module):
    ...
    def sinkhorn(self, cost_input: torch.Tensor, tol: float = 0.0001):
        """Sinkhorn based MoE routing function"""
        cost = torch.exp(cost_input)
        ...
        #black mamba init
        d0 = (self.tokens_per_batch / self.num_experts) * torch.exp(2 * cost_input).sum(axis=1)
