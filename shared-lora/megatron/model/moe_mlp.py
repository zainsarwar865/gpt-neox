...
class ParallelGroupedLoRas(torch.nn.Module):
    def __init__(
        ...
        _initialize_affine_weight_gpu(self.w2_A, output_layer_init_method, partition_dim=0, stride=stride)
        ...
        _initialize_affine_weight_gpu(self.w2_B, output_layer_init_method, partition_dim=0, stride=stride)
...
class ParallelGroupedMLP(torch.nn.Module):
    ...
    def permute_and_compute(
        ...
        #input_x = input_.view(-1, input_.shape[-1])
        ...
        # Replace with loRa computations here
    ...
    def forward(self, x: torch.Tensor, tokens_per_expert: torch.Tensor):
        ...
        lora_indices = lora_indices.flatten() + offset_vector
        ...
        #print(f"Norm X_1 : {x.norm()}, Norm x_1_loras : {x_1_loras.norm()}")
        #self.out_norms[0] = x.norm().detach().cpu().item()
        
        # Lambda balancing
        #scaled_x = self.lambda_scale(lambda_e, x, x_1_loras)
        ...
        # lora 2
        x_2_loras = self.permute_and_compute(
            x,
            tokens_per_lora,
            indices,
            lora_ids,
        ...
        #print(f"Norm X_2 : {x.norm()}, Norm x_2_loras : {x_2_loras.norm()}")
        # Log norms
        x = gg.ops.gmm(x, w2, grouped_gemm_batch_sizes)
        # self.out_norms[1] = x.norm().detach().cpu().item()
        # self.out_norms[2] = x_1_loras.norm().detach().cpu().item()
        # self.out_norms[3] = x_2_loras.norm().detach().cpu().item()
        #print(f"Norm X_1 : {self.out_norms[0]}, Norm x_1_loras : {self.out_norms[1]} , Norm X_2 : {self.out_norms[2]}, Norm x_2_loras : {self.out_norms[3]}")

        # lambda balancing
        #scaled_y = self.lambda_scale(lambda_e, x, x_2_loras)
        #scaled_y = x + x_2_loras
