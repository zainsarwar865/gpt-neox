...
from .moe_mlp_hetro import ParallelGroupedMLP, ParallelDroplessMinion
...
class ParallelDroplessMLP(torch.nn.Module):
    ...
    def __init__(
        ...
        self.minion_experts = ParallelDroplessMinion(neox_args, init_method, output_layer_init_method)
    ...
    def permute_and_compute(
        ...
        minion_tokens = tokens_per_expert[-self.num_minion_experts:].sum()
        exp_tokens =  tokens_per_expert[:self.num_experts - self.num_minion_experts].sum()
        # print(f"original t_p_e: {tokens_per_expert}")
        tokens_per_expert = tokens_per_expert[:len(tokens_per_expert) - self.num_minion_experts ]
        local_tokens_per_expert = get_expert_token_counts_for_rank(tokens_per_expert)
        # print(f"original input_parallel: {input_parallel.shape}")
        # print("minion_tokens", minion_tokens)
        if minion_tokens != 0:

            input_parallel, minion_input_parallel = torch.split(input_parallel, [exp_tokens, minion_tokens])
            # print("input_parallel.shape, minion_input_parallel.shape, minion_tokens", input_parallel.shape, minion_input_parallel.shape, minion_tokens)
            # exit()
            # print(f"new input_parallel: {input_parallel.shape}, minion_parallel: {minion_input_parallel.shape}, minion_tokens: {minion_tokens}, tokens_per_expert: {tokens_per_expert}")
            # exit()
            # Split into Regular | Minion experts
            ...
            # print(f"new input_parallel: {input_parallel.shape}, tokens_per_expert: {tokens_per_expert}")
            # exit()
            # Perform the expert computation for this rank's experts
            output_parallel = self.mlp(input_parallel, local_tokens_per_expert)
        
            # Perform the minion computation for this rank's experts
            # print("minion_input_parallel.shape", minion_input_parallel.shape)
            minion_output_parallel = self.minion_experts(minion_input_parallel)

            # Cat expert and minion computations
            output_parallel = torch.cat((output_parallel, minion_output_parallel[0]), dim=0)
        else:
            output_parallel = self.mlp(input_parallel, local_tokens_per_expert)
