...
class NeoXArgsLRScheduler(NeoXArgsTemplate):
    ...
    """
    an extra string to add to the dir_str that helps differentiate between models
    """
    ...
    """
    Use checkpoint to set the values of the scheduler (learning rate, waget_deepspeed_main_argsrmup iterations, minimum learning rate, maximum number of iterations, and decay style from checkpoint and ignore input arguments.
    """
...
class NeoXArgsMoE(NeoXArgsTemplate):
    ...
    """
    """
    Coefficient for MoE aux loss
    not used if set to None
    """

    """
    Coefficient for MoE z loss.
    not used if set to None
    """
    ...
    """
    moe_shared_expert_hidden_size: int = None
    """
    intermediate hidden dimension for the shared expert
    not used if set to None
    """
    ...
    """
    if false, shared expert is disabled
    """
    moe_log_counts_interval: int = 1000
    """
    the step at which to log the expert routing counts
    """