...
class ParallelBiasNetDistMLP(torch.nn.Module):
    def __init__(
        self,
        neox_args: NeoXArgs,
        init_method,
        ...
        """
        super(ParallelBiasNetDistMLP, self).__init__()
        ...
        self.hidden_size = neox_args.hypernetwork_bottleneck_dim + neox_args.hidden_size
        ...
        self.output_size = 4 * neox_args.hidden_size
        ...
        # print("self.num_rows_per_rank", self.num_rows_per_rank)
        ...
        _initialize_affine_weight_gpu(
            self.w_var, output_layer_init_method, partition_dim=0, stride=stride
        )
        ...
        _initialize_affine_weight_gpu(
            self.w_mean, output_layer_init_method, partition_dim=0, stride=stride
        )
    ...
    def forward(self, x: torch.Tensor):
        # Make sure tokens_per_expert is just a 1 tensor 
        tokens_per_expert = torch.tensor([x.shape[0]]).to(device=x.device)
        #print("Bias net tokens_per_expert shape is ", tokens_per_expert.shape)
        grouped_gemm_batch_sizes = tokens_per_expert.cpu().to(torch.long)
        w1, w_var, w_mean = (self.scale_grad(self.w1), self.scale_grad(self.w_var), self.scale_grad(self.w_mean))
        # Re-shape the weights for the grouped GEMMs
        w1 = w1.view(self.experts_per_rank, -1, self.hidden_size)
        w_var = w_var.view(self.experts_per_rank, -1, self.output_size)
        w_mean = w_mean.view(self.experts_per_rank, -1, self.output_size)

        # Compute the MLP
        x = gg.ops.gmm(x, w1, grouped_gemm_batch_sizes, trans_b=True)
        x = self.activation_func(x)

        out_var = gg.ops.gmm(x, w_var, grouped_gemm_batch_sizes)
        out_mean = gg.ops.gmm(x, w_mean, grouped_gemm_batch_sizes)
        z = self.reparameterization(out_mean, torch.exp(0.5 * out_var))
        return z

    def reparameterization(self, mu, var):
        eps = torch.randn_like(var).to(device=mu.device)
        z = mu + var*(eps)
        return z
...
class ParallelBiasNetMLP(torch.nn.Module):
    def __init__(
        self,
        neox_args: NeoXArgs,
        init_method,
        ...
        Copied from SparseMLP
        """
        super(ParallelBiasNetMLP, self).__init__()
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Re-shape the weights for the grouped GEMMs
        w1 = w1.view(self.experts_per_rank, -1, self.hidden_size)
        w2 = w2.view(self.experts_per_rank, -1, self.output_size)
...
class ParallelGroupedMLP(torch.nn.Module):
    def __init__(
        ...
        self.hn_down = torch.nn.Parameter(
        ...
        _initialize_affine_weight_gpu(
            self.hn_down, init_method, partition_dim=0, stride=1)

        self.hn_up = torch.nn.Parameter(
                torch.empty(neox_args.hidden_size + neox_args.intermediate_size,
                            neox_args.hypernetwork_bottleneck_dim,
                        device=torch.cuda.current_device(),
                        dtype=neox_args.params_dtype,
        ...
        _initialize_affine_weight_gpu(
            self.hn_up, init_method, partition_dim=0, stride=1)

        # output
        self.hn_up_w1 = torch.nn.Parameter(
            torch.empty(
                self.hidden_size + neox_args.hypernetwork_bottleneck_dim + neox_args.hypernetwork_bottleneck_dim,
                self.hidden_size * 5,
                device=torch.cuda.current_device(),
                dtype=neox_args.params_dtype,
            )
        ...
        _initialize_affine_weight_gpu(
            self.hn_up_w1, output_layer_init_method, partition_dim=0, stride=stride
        )

        self.hn_up_w2 = torch.nn.Parameter(
            torch.empty(
                self.hidden_size + neox_args.hypernetwork_bottleneck_dim + neox_args.hypernetwork_bottleneck_dim,
                self.hidden_size * 5,
                device=torch.cuda.current_device(),
        ...
        _initialize_affine_weight_gpu(
            self.hn_up_w2, output_layer_init_method, partition_dim=0, stride=stride
        )
    ...
    def forward(self, x: torch.Tensor, tokens_per_expert: torch.Tensor, bin_ids: torch.Tensor):
        ...
        if self.args.iteration > 0:
            ...
            w1_emb = torch.matmul(w1_summed, self.hn_down)
            ...
            w2_emb = torch.matmul(w2_summed, self.hn_up)
            ...
            # go from embeddings to w1_lowrank, w2_lowrank
            w1_lowrank = torch.matmul(emb, self.hn_up_w1)
            w2_lowrank = torch.matmul(emb, self.hn_up_w2)

            # split up w1_lowrank, w2_lowrank into their row/cols
            w1_A = w1_lowrank[:, :self.hidden_size]
            w1_B = w1_lowrank[:, self.hidden_size:]
            w2_A = w2_lowrank[:, self.hidden_size:]
            w2_B = w2_lowrank[:, :self.hidden_size]

            # x_hn = torch.einsum('ij,ik,ij->ik', w1_lowrank_row, w1_lowrank_col, x)
            x_hn = (x @ w1_A.T) @ w1_B
            # some initial stuff
            grouped_gemm_batch_sizes = tokens_per_expert.cpu().to(torch.long)
            w1, w2 = (self.scale_grad(self.w1), self.scale_grad(self.w2))
            ...
            x_e = gg.ops.gmm(x, w1, grouped_gemm_batch_sizes, trans_b=True)
            
            x = self.activation_func(x_e + x_hn)
            y_e = gg.ops.gmm(x, w2, grouped_gemm_batch_sizes)
            # y_hn = torch.einsum('ij,ik,ij->ik', w2_lowrank_row, w2_lowrank_col, x)
            y_hn = (x @ w2_A.T) @ w2_B
            return y_e + y_hn
        else:
            # some initial stuff
